...
type cwVerifier func(cw.ChargifyWebhook) error

type payloadTestInput struct {
	TestType   string
	InputValue string
	Verifier   cwVerifier
}

//Sample Full Body from SubscriptionStateChange message
...
var payloadInputs = []payloadTestInput{
	payloadTestInput{
		"balance in cents",
		"payload[subscription][balance_in_cents]=9900",
		func(cw cw.ChargifyWebhook) error {
			s := cw.Payload["subscription"].(map[string]interface{})
			if s["balance_in_cents"] != "9900" {
				return errors.New(fmt.Sprintf("Failed to unmarshal payload[subscription][balance_in_cents], got: %v\n", s["balance_in_cents"]))
			}
			return nil
		},
	},
	payloadTestInput{
		"activated at",
		"payload[subscription][activated_at]=2012-09-09%2011%3A38%3A33%20-0400",
		func(cw cw.ChargifyWebhook) error {
			s := cw.Payload["subscription"].(map[string]interface{})
			if s["activated_at"] != "2012-09-09 11:38:33 -0400" {
				return errors.New(fmt.Sprintf("Failed to unmarshal payload[subscription][activated_at], got: %v\n", s["activated_at"]))
			}
			return nil
		},
	},
	payloadTestInput{
		...
		func(cw cw.ChargifyWebhook) error {
			...
			if _, ok := s["canceled_at"]; ok {
				return errors.New(fmt.Sprintf("Failed to unmarshal payload[subscription][canceled_at], should ignore empty value, got: %v\n", s["canceled_at"]))
			}
			return nil
		},
	},
...
func TestParseChargifyWebhook(t *testing.T) {
	body := "id=188397125&event=subscription_state_change"
	var (
